...
pub struct CardDetails {
    ...
    pub regulated_status: Option<CardDetailsRegulatedStatus>,
...
/// An enum representing the possible values of an `CardDetails`'s `regulated_status` field.
#[derive(Copy, Clone, Debug, Deserialize, Serialize, Eq, PartialEq)]
#[serde(rename_all = "snake_case")]
pub enum CardDetailsRegulatedStatus {
    Regulated,
    Unregulated,
}

impl CardDetailsRegulatedStatus {
    pub fn as_str(self) -> &'static str {
        match self {
            CardDetailsRegulatedStatus::Regulated => "regulated",
            CardDetailsRegulatedStatus::Unregulated => "unregulated",
        }
    }
}

impl AsRef<str> for CardDetailsRegulatedStatus {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}

impl std::fmt::Display for CardDetailsRegulatedStatus {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        self.as_str().fmt(f)
    }
}
impl std::default::Default for CardDetailsRegulatedStatus {
    fn default() -> Self {
        Self::Regulated
    }
}
