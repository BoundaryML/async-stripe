...
/// An enum representing the possible values of an `Refund`'s `pending_reason` field.
#[derive(Copy, Clone, Debug, Deserialize, Serialize, Eq, PartialEq)]
#[serde(rename_all = "snake_case")]
...
impl RefundPendingReason {
    pub fn as_str(self) -> &'static str {
        match self {
            RefundPendingReason::ChargePending => "charge_pending",
            RefundPendingReason::InsufficientFunds => "insufficient_funds",
            RefundPendingReason::Processing => "processing",
        }
    }
}

impl AsRef<str> for RefundPendingReason {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}
...
impl std::default::Default for RefundPendingReason {
    fn default() -> Self {
        Self::ChargePending
    }
}
